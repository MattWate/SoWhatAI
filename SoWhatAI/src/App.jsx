          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.map(p => (
              <div key={p.id} className="p-4 bg-gray-800/60 border border-gray-700 rounded-lg">
                <div className="text-white font-semibold">{p.name || 'Untitled Project'}</div>
  ...
  return (
    <div className="w-full bg-gray-900/50 backdrop-blur-lg border border-gray-700/50 rounded-lg shadow-2xl p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => onDownload(results)} className="inline-flex items-center px-4 py-2 text-sm rounded-md text-white bg-green-600 hover:bg-green-700">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
          </svg>
          Download Report
...
const AnalysisToolPage = ({ onNavigate, initialProjectId, onSavedProjectId }) => {
      ...
      try {
        if (initialProjectId) {
          ...
          const created = await createProject({
            name: results?.researchQuestion?.slice(0, 60) || `Project ${new Date().toLocaleString()}`,
            analysis_report: {
              ...results,
              dataSet: dataSet.map(f => ({ name: f.name, type: f.type }))
            }
  ...
  const handleDownloadReport = (_results) => { /* TODO: integrate your PDF export or hook to ReportViewerPage */ };
