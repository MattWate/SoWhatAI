...
const MappingModal = ({ file, onClose, onSave }) => {
    ...
    useEffect(() => {
        if (!window.Papa || !window.XLSX) return; // Guard against libraries not being loaded
        setIsLoading(true);
...
const ConfigurationPage = ({ dataSet, setDataSet, onAnalyze, onBack, error }) => {
    ...
    useEffect(() => {
        const needsCSVParsing = dataSet.some(f => /\.csv$/i.test(f.name));
        ...
        const checkLibraries = () => {
            ...
            if (papaReady && xlsxReady && mammothReady) {
                ...
                timerId = setTimeout(checkLibraries, 100);
            }
        };
        checkLibraries();
    ...
    const handleComponentChange = (e) => {
        ...
        setReportConfig(prev => ({
            ...prev,
            components: { ...prev.components, [name]: checked }
        }));
    };

    return (<div className="bg-gray-900/50 backdrop-blur-lg border border-gray-700/50 rounded-lg shadow-2xl p-6 space-y-6"><button onClick={onBack} className="text-sm font-medium text-[#13BBAF] hover:text-teal-400">&larr; Back to upload</button><div><h2 className="text-2xl font-semibold text-white">Step 2: Configure Your Analysis</h2><p className="text-sm text-gray-400">Provide your research goals to guide the AI analysis.</p></div>
    
    <div><label htmlFor="research-question" className="block text-lg font-semibold text-white">1. Research Question</label><p className="text-sm text-gray-400 mb-2">What is the primary question you want this analysis to answer?</p><textarea id="research-question" rows={3} className="shadow-sm focus:ring-[#13BBAF] focus:border-[#13BBAF] block w-full sm:text-sm border-gray-600 bg-gray-800 text-white rounded-md p-2" placeholder="e.g., How do our power-users feel about the new interface performance?" value={researchQuestion} onChange={(e) => setResearchQuestion(e.target.value)} /></div>

    <div><label htmlFor="report-focus" className="block text-lg font-semibold text-white">2. Report Focus & Context (Optional)</label><p className="text-sm text-gray-400 mb-2">Provide any specific context or areas for the AI to focus on.</p><textarea id="report-focus" rows={3} className="shadow-sm focus:ring-[#13BBAF] focus:border-[#13BBAF] block w-full sm:text-sm border-gray-600 bg-gray-800 text-white rounded-md p-2" placeholder="e.g., Focus on all mentions of cyber security. or This data is from support tickets; summarise the main issues." value={reportConfig.focus} onChange={(e) => setReportConfig(p => ({ ...p, focus: e.target.value }))} /></div>

    {isDataReady ? (<div className="space-y-4"><div><h3 className="text-lg font-semibold text-white">3. Configure Data Files</h3><p className="text-sm text-gray-400 mb-2">Map columns for each uploaded spreadsheet.</p><div className="space-y-2">{dataSet.map(file => (<div key={file.id} className="flex items-center justify-between p-3 bg-gray-800/70 rounded-md"><span className="font-medium text-gray-300 truncate">{file.name}</span>{file.type === 'spreadsheet' && (<button onClick={() => setModalFileId(file.id)} className="inline-flex items-center px-3 py-1 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-[#13BBAF] hover:bg-teal-600 transition-colors">Map Columns</button>)}{file.type === 'text' && (<span className="text-sm text-green-400">Ready to Analyse</span>)}</div>))}</div></div>
    <div><h3 className="text-lg font-semibold text-white">4. Report Components</h3><p className="text-sm text-gray-400 mb-2">Select the sections you want to include in the final report.</p><div className="flex flex-wrap gap-x-6 gap-y-2">{Object.keys(reportConfig.components).map(key => (<div key={key} className="flex items-center"><input id={key} name={key} type="checkbox" checked={reportConfig.components[key]} onChange={handleComponentChange} className="h-4 w-4 text-teal-600 bg-gray-700 border-gray-600 rounded focus:ring-teal-500" /><label htmlFor={key} className="ml-2 block text-sm text-gray-300 capitalize">{key === 'soWhat' ? 'So What?' : key}</label></div>))}</div></div></div>) : (<div className="text-center text-gray-400 p-8 bg-gray-800/50 rounded-md"><div className="flex justify-center items-center"><svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg><span>Preparing Data Set...</span></div></div>)}<div className="pt-5"><div className="flex justify-end"><button onClick={() => onAnalyze(researchQuestion, reportConfig)} className="w-full md:w-auto inline-flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 transition-colors transform hover:scale-105">Analyse Full Data Set</button></div>{error && <p className="text-red-400 text-sm mt-4 text-right">{error}</p>}</div>{modalFile && (<MappingModal file={modalFile} onClose={() => setModalFileId(null)} onSave={handleMappingsUpdate} />)}</div>);
};
...
const AnalysisReportPage = ({ dataSet, onBack, results, onDownload }) => {
    ...
    const ResearchQuestionDisplay = ({ question }) => (question && <div className="p-4 rounded-lg border border-gray-700 bg-gray-800/50 backdrop-blur-sm mb-6"><h3 className="text-lg font-semibold text-white">Research Question</h3><p className="mt-2 text-gray-300 italic">"{question}"</p></div>);
    ...
    const handleAnalysis = async (researchQuestion, reportConfig) => {
        ...
        try {
            ...
            const response = await fetch('/.netlify/functions/analyze', {
                ...
                body: JSON.stringify({ textData: combinedText, quantitativeData: quantitativePayload, researchQuestion, reportConfig })
            });
