...
const generatePPTX = (results) => {
  ...
  // Set Metadata
  pres.title = results.researchQuestion || "Research Report";
  pres.subject = "AI Generated Insights";
  pres.layout = 'LAYOUT_16x9';

  // --- Slide 1: Title Slide ---
  let slide = pres.addSlide();
  slide.addText("Research Insights Report", { x: 0.5, y: '40%', w: '90%', fontSize: 36, fontFace: "Arial", color: "363636", bold: true });
  slide.addText(results.researchQuestion || "Generated by SoWhatAI", { x: 0.5, y: '55%', w: '90%', fontSize: 18, fontFace: "Arial", color: "787878" });
  slide.addText(`Generated: ${new Date().toLocaleDateString()}`, { x: 0.5, y: '90%', fontSize: 10, color: "BBBBBB" });

  // --- Slide 2: Narrative Overview ---
  if (results.narrativeOverview) {
    slide = pres.addSlide();
    slide.addText("Executive Summary", { x: 0.5, y: 0.5, w: '90%', fontSize: 24, bold: true, color: "13BBAF" });
    slide.addText(results.narrativeOverview, { x: 0.5, y: 1.5, w: '90%', h: '75%', fontSize: 14, color: "363636", lineSpacing: 24, valign: 'top' });
  }

  // --- Slide 3: So What? / Recommendations ---
  if (results.soWhatActions && results.soWhatActions.length > 0) {
    slide = pres.addSlide();
    slide.addText("So What? Actions & Recommendations", { x: 0.5, y: 0.5, w: '90%', fontSize: 24, bold: true, color: "13BBAF" });
    
    results.soWhatActions.forEach((action, index) => {
  ...
  // --- Theme Slides (Iterate through Source Types and Themes) ---
  if (results.analysisBySource) {
    results.analysisBySource.forEach(source => {
      if (!source.themes) return;
      
      source.themes.forEach(theme => {
        slide = pres.addSlide();
        
        // Header
        slide.addText(`${theme.emoji || ''} Theme: ${theme.theme}`, { x: 0.5, y: 0.5, w: '90%', fontSize: 22, bold: true, color: "363636" });
        slide.addText(`Source: ${source.sourceType}`, { x: 0.5, y: 0.9, fontSize: 10, italic: true, color: "888888" });

        // Narrative
        slide.addText(theme.themeNarrative || "No narrative available.", { x: 0.5, y: 1.2, w: '90%', fontSize: 14, color: "363636" });

        // Drivers & Barriers (Left Column)
        slide.addText("Key Drivers", { x: 0.5, y: 2.8, fontSize: 12, bold: true, color: "13BBAF" });
        if (theme.drivers && theme.drivers.length > 0) {
           slide.addText(theme.drivers.join('\n'), { x: 0.5, y: 3.1, w: 4.5, h: 2.0, fontSize: 11, bullet: true, valign: 'top' });
        }

        slide.addText("Barriers", { x: 0.5, y: 5.0, fontSize: 12, bold: true, color: "E76F51" });
        if (theme.barriers && theme.barriers.length > 0) {
           slide.addText(theme.barriers.join('\n'), { x: 0.5, y: 5.3, w: 4.5, h: 2.0, fontSize: 11, bullet: true, valign: 'top' });
        }

        // Evidence/Quotes (Right Column)
        slide.addText("Supporting Evidence", { x: 5.5, y: 2.8, fontSize: 12, bold: true, color: "13BBAF" });
        if (theme.evidence && theme.evidence.length > 0) {
          theme.evidence.forEach((quote, i) => {
            slide.addText(`"${quote}"`, { x: 5.5, y: 3.1 + (i * 1.0), w: '40%', fontSize: 11, italic: true, color: "555555" });
          });
        }
      });
    });
  ...
  // --- Quantitative Slides ---
  if (results.quantitativeResults && results.quantitativeResults.length > 0) {
    results.quantitativeResults.forEach(fileResult => {
      if (fileResult.stats && fileResult.stats.length > 0) {
        slide = pres.addSlide();
        slide.addText(`Data Analysis: ${fileResult.sourceFile}`, { x: 0.5, y: 0.5, w: '90%', fontSize: 24, bold: true, color: "13BBAF" });
        
        fileResult.stats.forEach((stat, i) => {
           const yPos = 1.5 + (i * 1.5);
           slide.addText(stat.title, { x: 0.5, y: yPos, w: '90%', fontSize: 16, bold: true });
           slide.addText(
             `Mean: ${stat.mean} | Median: ${stat.median} | Mode: ${stat.mode}`, 
             { x: 0.5, y: yPos + 0.4, w: '90%', fontSize: 14, color: "555555" }
           );
        });
      }
    });
...
const AnalysisToolPage = ({ user, onNavigate }) => {
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      const pres = generatePPTX(analysisResults);
      await pres.writeFile({ fileName: `SoWhatAI-Report-${Date.now()}.pptx` });
    } catch (err) {
