...
function createUnavailableBestPracticesData({
  startUrl,
  strategy = 'mobile',
  reason = 'unknown',
  message = ''
...
async function runBestPracticesEngine({ startUrl, strategy = 'mobile', psiResult = null } = {}) {
  try {
    if (!psiResult || psiResult.status !== 'success' || !psiResult.data) {
      const reason = psiResult?.error || 'unknown';
      const message = sanitizeErrorMessage(psiResult?.message || 'PSI best-practices data unavailable.');
      return {
        status: 'failed',
